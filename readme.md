# Projet StreamVault (MongoDB Niveau 1)

## Contexte du Projet
Ce dépôt contient la première étape du projet mandaté par **StreamVault**, une plateforme de streaming vidéo. L'objectif global est de structurer un catalogue de films hétérogènes qu'une base relationnelle classique ne pourrait pas absorber efficacement[cite: 1].

Ce répertoire correspond au **Niveau 1 (Imiter)** : une prise en main technique de MongoDB .
...
## Jeu de données
Le jeu de données initial est un fichier JSON (`livres-6aa12fece77f5652539886.json`) contenant une collection de 8 documents littéraires. Ce dataset a été choisi pour sa structure hétérogène (champs uniques, tableaux imbriqués pour les chapitres et les auteurs multiples).

## Contenu du Script (Playground)
Le fichier `playground-1.mongodb.js` contient l'ensemble des requêtes NoSQL répondant aux 20 questions du brief[cite: 1]. Il couvre les opérations CRUD (Create, Read, Update, Delete) suivantes :
...
Attributs typés rattachés aux entités (tous configurés comme *Functional*) :
...
3. Activer un raisonneur (ex: *Pellet* ou *HermiT*) depuis le menu `Reasoner` pour vérifier la cohérence du graphe et propager les propriétés transitives.
